...
export class GenesisPayApiError extends Error {
  ...
  /** Saved purchase identity, attached by pay() even when no response arrived. */
...
/** The retained payment is terminally failed; strict replay may return it with HTTP 200. */
...
/**
 * `pay({ waitForOutcome })` ran out of its wait budget while the payment was
 * still `unresolved`, `approved` or `executing`.
 *
 * Not confirmed yet is not failed (MR-306): the payment may already have been
 * charged and may still settle. Keep polling `paymentStatus(paymentId)`; retry
 * the purchase only with the same `idempotencyKey`, never with a new one. A
 * subclass of `GenesisPayPaymentOutcomeUnknownError` with the same `code`, so
 * existing retry logic that branches on the parent keeps working.
 */
export class GenesisPayOutcomeWaitTimeoutError extends GenesisPayPaymentOutcomeUnknownError {
  declare readonly payment: AgentPaymentRecord;
  /** How long the client waited after the pay response, in milliseconds. */
  readonly waitedMs: number;

  constructor(
    message: string,
    options: { payment: AgentPaymentRecord; idempotencyKey?: string | null; waitedMs: number },
  ) {
    super(message, {
      status: 0,
      payment: options.payment,
    ...
    this.name = "GenesisPayOutcomeWaitTimeoutError";
...
/** The account/key namespace already identifies different or unverifiable request terms. */
export class GenesisPayIdempotencyConflictError extends GenesisPayApiError {
  readonly paymentId: string | null;
  readonly payment: AgentPaymentRecord | null;
  constructor(message: string, options: { paymentId?: string | null; payment?: AgentPaymentRecord | null } = {}) {
    super(message, { status: 409, code: "idempotency_conflict" });
    this.name = "GenesisPayIdempotencyConflictError";
    this.paymentId = options.paymentId ?? options.payment?.id ?? null;
    this.payment = options.payment ?? null;
  }
}
