...
import { assertFulfillmentEventEvidence, GENESISPAY_API_VERSION, type ExpectedDeliveryContract, type NetworkContract, type FeeCollectionStatus } from "./fulfillment.js";
...
export type GenesisPayMandateEventData = {
  /** Present for contract events. `mandate` is a projection-time snapshot, not the historical event state. */
  protocol?: "contract_mandate_v1";
  mandateStateContext?: { kind: "projection_snapshot"; observedAt: string };
  /** Immutable chain event facts; occurredAt is its block timestamp. Deliveries may be out of order. */
  contractEvent?: {
    kind: "activation" | "revocation" | "subscription_charge" | "per_use_charge";
    chainId: number; executor: string; transactionHash: string; blockHash: string; blockNumber: string;
    logIndex: number; logicalEventIdentity: string; sequence: string | null;
    grossMinor: string; sellerMinor: string; feeMinor: string; nextEligibleAt: string | null;
  };
  mandate: {
    id: string;
    kind: string;
...
/** Signed snapshot only. Fresh fulfillment.verify is required before credit. */
export type GenesisPayFulfillmentEventData = {
  schema: "genesispay.fulfillment-evidence";
  version: typeof GENESISPAY_API_VERSION;
  authorityVersion: typeof GENESISPAY_API_VERSION;
  sellerId: string;
  attemptId: string;
  linkId: string;
  clientReferenceId: string | null;
  entitlementId: string | null;
  entitlement: { expiresAt: string; revokedAt: string | null; valid: boolean } | null;
  paymentChannel: "checkout" | "x402" | "bank_transfer";
  ...
  settlement: {
    strategy: "dual" | "atomic";
    grossAmountMinor: string;
    sellerAmountMinor: string;
    destination: `0x${string}`;
    feeTerms: { type: "record_only" | "payer_authorized"; amountMinor: string; treasuryAddress: `0x${string}` | null };
    feeCollection: { status: FeeCollectionStatus; amountMinor: string | null; treasuryAddress: `0x${string}` | null; transactionHash: `0x${string}` | null; collectedAt: string | null };
  };
  product: { contractVersion: typeof GENESISPAY_API_VERSION; productId: string; sku: string | null; delivery: ExpectedDeliveryContract } | null;
};

export type GenesisPayFulfillmentEvent = GenesisPayEventEnvelope<"payment.fulfilled", GenesisPayFulfillmentEventData> & {
...
export async function constructEvent(
  ...
  if (envelope.type === "payment.fulfilled") {
    const fulfilled = parsed as Record<string, unknown>;
    const data = envelope.data;
    if (fulfilled.apiVersion !== GENESISPAY_API_VERSION || typeof fulfilled.livemode !== "boolean" ||
      !(data.clientReferenceId === null || typeof data.clientReferenceId === "string")) {
      throw new GenesisPaySignatureVerificationError("Invalid payment.fulfilled envelope.");
    }
    try { assertFulfillmentEventEvidence(data); } catch {
      throw new GenesisPaySignatureVerificationError("Invalid payment.fulfilled evidence snapshot.");
    }
    const network = data.network as NetworkContract;
    if (fulfilled.livemode !== (network.mode === "live")) {
      throw new GenesisPaySignatureVerificationError("Inconsistent payment.fulfilled livemode.");
    }
  }
