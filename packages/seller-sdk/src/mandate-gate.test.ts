...
describe("createMandateGate", () => {
  ...
  it("charges the mandate and runs the handler on success", async () => {
    ...
    expect(response.headers.get("PEERPAY-MANDATE-CHARGE")).toBe("c1");
    expect(handler).toHaveBeenCalledTimes(1);

    const [chargeUrl, chargeInit] = fetchFn.mock.calls[0] as [string, RequestInit];
    expect(chargeUrl).toBe("https://peerpay.example/api/v1/mandates/mandate-1/charge");
    expect(chargeInit.headers).toMatchObject({
      Authorization: "Bearer pp_sk_test",
    });
  ...
  it("maps unreachable PeerPay to a 402 with a clear error", async () => {
