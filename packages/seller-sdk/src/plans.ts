// `peerpay.plans.*` — subscription plans over `/api/v1/plans`.
//
...
// else answers 404 (PeerPayNotFoundError) rather than 403 — the API is no
    ...
    list: async () => {
      ...
      if (!Array.isArray(plans)) {
        ...
        throw new PeerPayConfigError(
          `PeerPay GET /api/v1/plans returned no plans array: ${JSON.stringify(body)}`,
        );
      }
...
function notFoundMessage(id: string): string {
  return (
    `No PeerPay subscription plan with publicId "${id}". Check the id and that ` +
...
function requirePlan(body: unknown, operation: string): SubscriptionPlan {
  ...
  if (!raw || typeof raw.publicId !== "string" || !raw.publicId) {
    throw new PeerPayConfigError(
      `PeerPay ${operation} returned no plan: ${JSON.stringify(body)}`,
    );
  }
