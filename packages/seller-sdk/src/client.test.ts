...
import { DEFAULT_FACILITATOR_BASE_URL } from "./genesispay-settlement.js";

const TEST_KEY = `gp_sk_test_${"a".repeat(32)}`;
const LIVE_KEY = `gp_sk_live_${"a".repeat(32)}`;
const LEGACY_KEY = `gp_sk_${"a".repeat(32)}`;
const receiving = "0x1111111111111111111111111111111111111111";
const other = "0x2222222222222222222222222222222222222222";
const CREATION_OPTIONS = { idempotencyKey: "checkout-test-create" } as const;
...
const STRICT_REQUEST_ID = "req_strict_checkout";

/**
 * A backend that actually answered the version this SDK asked for. Creating a
 * payable link now requires that answer, so every `checkout.create` fixture
 * speaks it — the un-upgraded backend has its own test below.
 */
const strictFetch = (value: object, status = 201) =>
  vi.fn(
    async () =>
      new Response(
        JSON.stringify({
          ...value,
          requestId: STRICT_REQUEST_ID,
        ...
        }),
        {
          ...
          headers: {
            ...
            "GENESISPAY-Request-Id": STRICT_REQUEST_ID,
...
describe("GenesisPay constructor", () => {
  it("parses the mode from the key prefix (test / live / legacy)", () => {
    expect(new GenesisPay({ apiKey: TEST_KEY }).mode).toBe("test");
    expect(new GenesisPay({ apiKey: LEGACY_KEY }).mode).toBe("test");
    expect(new GenesisPay({ apiKey: LIVE_KEY, baseUrl: "https://prod.example" }).mode).toBe("live");
  });

  it("defaults a test key to the dev facilitator with zero config", () => {
    expect(new GenesisPay({ apiKey: TEST_KEY }).baseUrl).toBe(DEFAULT_FACILITATOR_BASE_URL);
  });

  it("defaults a live key to the production facilitator with zero config", () => {
    expect(new GenesisPay({ apiKey: LIVE_KEY }).baseUrl).toBe("https://genesispay.finance");
  });

  it("rejects a non-GenesisPay key", () => {
    expect(() => new GenesisPay({ apiKey: "sk_live_whatever" })).toThrow(GenesisPayConfigError);
  });

  it("enforces https except for localhost", () => {
    expect(() => new GenesisPay({ apiKey: TEST_KEY, baseUrl: "http://evil.example" })).toThrow(
      /https/,
    );
    expect(new GenesisPay({ apiKey: TEST_KEY, baseUrl: "http://localhost:3000" }).baseUrl).toBe(
      "http://localhost:3000",
    );
  });
});
...
describe("retrieveSeller", () => {
  it("is single-flight and TTL-cached", async () => {
    ...
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });
  ...
  it("never caches failures — clears the memo and retries", async () => {
    ...
    await expect(client.retrieveSeller()).rejects.toBeInstanceOf(GenesisPayConfigError);
  ...
  it("fails closed on an asset-integrity mismatch", async () => {
    ...
    await expect(client.retrieveSeller()).rejects.toBeInstanceOf(GenesisPayNetworkSafetyError);
  });
  ...
  it("fails closed with a typed safety error on an unknown network name", async () => {
    const bad = sellerConfig({
      network: {
        ...
        // GenesisPayNetworkSafetyError, not a generic lookup throw.
...
describe("checkout.create", () => {
  it("MR-207: refuses standalone reusable requests from JavaScript before HTTP", async () => {
    const fetchFn = jsonFetch({});
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });
    await expect(client.checkout.create({ title: "Request", amount: "1.00", linkType: "reusable" } as unknown as Parameters<GenesisPay["checkout"]["create"]>[0], CREATION_OPTIONS))
      .rejects.toThrow(/single-use/);
    expect(fetchFn).not.toHaveBeenCalled();
  });
  it("MR-1204: strict creation preserves item tax and idempotent authority negotiation", async () => {
    const taxConfig = { version: 1, treatment: "taxable", rateBps: 2000, note: null } as const;
    const fetchFn = strictFetch({ link: { publicId: "p_tax", payUrl: "https://pay.example/p_tax", amount: "1.20", destinationWallet: receiving, taxConfig } });
    const client = new GenesisPay({ apiKey: TEST_KEY, expectedPayTo: receiving, fetchFn: fetchFn as unknown as typeof fetch });
    const link = await client.checkout.create({ title: "Taxed request", amount: "1.20", taxConfig }, CREATION_OPTIONS);
    const [, init] = fetchFn.mock.calls[0] as unknown as [string, RequestInit];
    expect(JSON.parse(init.body as string)).toMatchObject({ taxConfig, amount: "1.20", linkType: "single" });
    expect(new Headers(init.headers).get("Idempotency-Key")).toBe(CREATION_OPTIONS.idempotencyKey);
    expect(new Headers(init.headers).get("GENESISPAY-Version")).toBe("2026-08-26");
    expect(link.taxConfig).toEqual(taxConfig);
    expect(client.checkoutDocuments).toBeDefined();
    expect(client.fulfillment).toBeDefined();
  });
  it("unwraps the nested { link } body and returns the server-normalized amount", async () => {
    const fetchFn = strictFetch({
    ...
    const requestHeaders = new Headers(
      (fetchFn.mock.calls[0] as unknown as [string, RequestInit])[1].headers,
    );
    expect(requestHeaders.get("idempotency-key")).toBe(CREATION_OPTIONS.idempotencyKey);
    expect(requestHeaders.get("GENESISPAY-Version")).toBe("2026-08-26");
  ...
  it("fails closed when expectedPayTo is set and the response omits destinationWallet", async () => {
    // Bypass-by-omission: the pin used to silently skip a missing wallet, so
    // an intermediary that STRIPPED the field disabled the guarantee. Absent
    // is exactly what the pin cannot vouch for.
    const fetchFn = strictFetch({
      link: { publicId: "p1", payUrl: "https://pay.example/p1", amountUsdc: "5.00", title: "Credits" },
    });
    const client = new GenesisPay({
    ...
    await expect(
      client.checkout.create({ title: "Credits", amount: "5.00" }, CREATION_OPTIONS),
    ).rejects.toBeInstanceOf(GenesisPayNetworkSafetyError);
  });

  it("MR-202: refuses a payable link from a backend that ignored the version header", async () => {
    // The rollback/old-backend case. A pre-strict backend answers a perfectly
    // ordinary 201 with a working payUrl — and an attempt created against it
    // carries no authority provenance, so strict fulfillment could never verify
    // the payment. Handing that payUrl to a payer buys an unfulfillable
    // purchase, so the SDK refuses the link instead of the payment.
    const fetchFn = jsonFetch(
      { link: { publicId: "p1", payUrl: "https://pay.example/p1", amountUsdc: "5.00", title: "Credits" } },
      201,
    );
  ...
  it("refuses a link whose response metadata was stripped in transit", async () => {
    const fetchFn = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            link: { publicId: "p1", payUrl: "https://pay.example/p1", amountUsdc: "5.00" },
            requestId: "req_body",
            apiVersion: "2026-08-26",
          }),
          {
            ...
            headers: {
              "content-type": "application/json",
              "GENESISPAY-Version": "2026-08-26",
              "GENESISPAY-Request-Id": "req_header",
            },
          },
        ),
    );
  ...
  it("requires caller idempotency before a money-bearing create request", async () => {
    const fetchFn = strictFetch({ link: {} });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });

    // @ts-expect-error 1.0 deliberately has no idempotency-free creation overload.
    await expect(client.checkout.create({ title: "Credits", amount: "5.00" })).rejects.toBeInstanceOf(
      GenesisPayValidationError,
    );
    expect(fetchFn).not.toHaveBeenCalled();
  });
  ...
  it("omits the optional fields entirely when they are not set", async () => {
    ...
    await client.checkout.create({ title: "x", amountUsdc: "1.00" }, CREATION_OPTIONS);
  ...
  // MR-102 on the public surface: `amountUsdc` named a currency the value did
  // not always have — on an EURC link it carried euros. `amount` is the field
  // now; `amountUsdc` stays a working alias.
  it("MR-102: sends the amount under both names so a pre-0.6.0 backend still finds it", async () => {
    const fetchFn = strictFetch({
      link: { publicId: "p1", payUrl: "https://pay.example/p1", amount: "5.00", title: "x" },
    });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });

    await client.checkout.create({ title: "x", amount: "5.00" }, CREATION_OPTIONS);

    const body = JSON.parse(
      (fetchFn.mock.calls[0] as unknown as [string, RequestInit])[1].body as string,
    ) as Record<string, unknown>;
    expect(body.amount).toBe("5.00");
    expect(body.amountUsdc).toBe("5.00");
  });

  it("MR-102: still accepts the deprecated amountUsdc on its own", async () => {
    const fetchFn = strictFetch({
      link: { publicId: "p1", payUrl: "https://pay.example/p1", amount: "5.00", title: "x" },
    });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });

    await client.checkout.create({ title: "x", amountUsdc: "5.00" }, CREATION_OPTIONS);
  ...
  it("MR-102: accepts both fields when they are the same amount ('5.0' vs '5.00')", async () => {
    const fetchFn = strictFetch({
      link: { publicId: "p1", payUrl: "https://pay.example/p1", amount: "5.00", title: "x" },
    });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });
    ...
    await expect(
      client.checkout.create(
        { title: "x", amount: "5.0", amountUsdc: "5.00" },
        CREATION_OPTIONS,
      ),
    ).resolves.toMatchObject({ amount: "5.00" });
  });

  it("MR-102: refuses contradicting amount and amountUsdc without calling the API", async () => {
    const fetchFn = strictFetch({ link: {} });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });

    await expect(
      client.checkout.create(
        { title: "x", amount: "5.00", amountUsdc: "50.00" },
        CREATION_OPTIONS,
      ),
    ).rejects.toBeInstanceOf(GenesisPayValidationError);
    // The point of failing locally: a link for money the caller did not mean
    // must never be created, so the request is never sent at all.
    expect(fetchFn).not.toHaveBeenCalled();
  });

  it("MR-102: requires an amount under one of the two names", async () => {
    const fetchFn = strictFetch({ link: {} });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });

    // The union in CheckoutAmountInput makes this a compile error too — the
    // guarantee `amountUsdc: string` used to give is not traded for a runtime
    // check. `npm run typecheck` fails if this ever starts compiling.
    // @ts-expect-error neither `amount` nor `amountUsdc` is set
    await expect(client.checkout.create({ title: "x" }, CREATION_OPTIONS)).rejects.toBeInstanceOf(
      GenesisPayValidationError,
    );
    expect(fetchFn).not.toHaveBeenCalled();
  });

  it("preserves an historical EURC asset returned by the API", async () => {
    const fetchFn = strictFetch({
      link: {
        publicId: "p1",
        payUrl: "https://pay.example/p1",
    ...
    // Existing links remain readable even though new hosted checkout links
    // settle in USDC during the beta.
    expect(link).toMatchObject({ amount: "12.50", amountUsdc: "12.50", asset: "EURC" });
  });

  it("falls back to amountUsdc when the backend predates the `amount` field", async () => {
    const fetchFn = strictFetch({
      link: { publicId: "p1", payUrl: "https://pay.example/p1", amountUsdc: "5.00", title: "x" },
    });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });

    const link = await client.checkout.create({ title: "x", amount: "5.00" }, CREATION_OPTIONS);

    expect(link).toMatchObject({ amount: "5.00", amountUsdc: "5.00" });
  });

  it("throws GenesisPayConfigError on an unclassified non-2xx response", async () => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
      fetchFn: strictFetch({ error: "nope" }, 500) as unknown as typeof fetch,
    });

    await expect(
      client.checkout.create({ title: "x", amountUsdc: "1.00" }, CREATION_OPTIONS),
  ...
  it("throws when the created link has no payUrl", async () => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
      fetchFn: strictFetch({ link: { publicId: "p1" } }) as unknown as typeof fetch,
    });
...
describe("checkout.create — redirect URL validation", () => {
  const create = (input: Parameters<GenesisPay["checkout"]["create"]>[0]) => {
    const fetchFn = strictFetch({
      link: { publicId: "p1", payUrl: "https://pay.example/p1", amountUsdc: "1.00", title: "x" },
    });
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: fetchFn as unknown as typeof fetch });
    return { client, fetchFn, promise: client.checkout.create(input, CREATION_OPTIONS) };
  };

  const lastBody = (fetchFn: ReturnType<typeof jsonFetch>) =>
    JSON.parse(
      (fetchFn.mock.calls[0] as unknown as [string, RequestInit])[1].body as string,
    ) as Record<string, unknown>;

  it("accepts https and both loopback http forms", async () => {
    for (const url of [
      "https://shop.example/thanks",
      "http://localhost:3000/thanks",
      "http://127.0.0.1:3000/thanks",
    ]) {
      const { fetchFn, promise } = create({
        title: "x",
        amountUsdc: "1.00",
        returnUrl: url,
      });
      await promise;
      expect(lastBody(fetchFn).returnUrl).toBe(url);
    }
  });

  it("trims the redirect URLs before sending them", async () => {
    const { fetchFn, promise } = create({
      title: "x",
      amountUsdc: "1.00",
    ...
    expect(lastBody(fetchFn).returnUrl).toBe("https://shop.example/thanks");
    expect(lastBody(fetchFn).cancelUrl).toBe("https://shop.example/cart");
  });

  it("drops whitespace-only redirect fields rather than sending them", async () => {
    const { fetchFn, promise } = create({
      title: "x",
      amountUsdc: "1.00",
      returnUrl: "   ",
    ...
    expect(lastBody(fetchFn)).not.toHaveProperty("returnUrl");
  });

  it("rejects an unsafe scheme without calling the API", async () => {
    const { fetchFn, promise } = create({
      title: "x",
      amountUsdc: "1.00",
    ...
    await expect(promise).rejects.toBeInstanceOf(GenesisPayValidationError);
    expect(fetchFn).not.toHaveBeenCalled();
  });

  it("rejects remote http and deceptive localhost hostnames", async () => {
    for (const bad of [
      "http://shop.example/thanks",
      "http://localhost.example/thanks",
      "http://localhost.evil.com/thanks",
      "http://127.0.0.1.evil.com/thanks",
    ]) {
      const { fetchFn, promise } = create({
        title: "x",
        amountUsdc: "1.00",
      ...
      await expect(promise, bad).rejects.toBeInstanceOf(GenesisPayValidationError);
      expect(fetchFn, bad).not.toHaveBeenCalled();
    }
  });
  ...
  it("rejects a redirect URL over 2048 characters", async () => {
    const { fetchFn, promise } = create({
      title: "x",
      amountUsdc: "1.00",
      returnUrl: `https://shop.example/${"a".repeat(2048)}`,
    });
  ...
  it("reports both fields when both are invalid, with the field paths", async () => {
    const { fetchFn, promise } = create({
      title: "x",
      amountUsdc: "1.00",
      returnUrl: "javascript:alert(1)",
      cancelUrl: "http://evil.example/cart",
    });

    const error = await promise.catch((e: unknown) => e);

    expect(error).toBeInstanceOf(GenesisPayValidationError);
    expect((error as GenesisPayValidationError).issues).toEqual([
      { path: "returnUrl", message: "returnUrl must be an https URL (http is allowed for localhost only)" },
      { path: "cancelUrl", message: "cancelUrl must be an https URL (http is allowed for localhost only)" },
    ]);
    expect(fetchFn).not.toHaveBeenCalled();
  });

  it("rejects non-string redirect fields (null / number) as validation issues, not TypeErrors", async () => {
    for (const bad of [null, 123]) {
      const { fetchFn, promise } = create({
        title: "x",
        amountUsdc: "1.00",
        returnUrl: bad,
      } as unknown as Parameters<GenesisPay["checkout"]["create"]>[0]);

      const error = await promise.catch((e: unknown) => e);

      expect(error, String(bad)).toBeInstanceOf(GenesisPayValidationError);
      expect((error as GenesisPayValidationError).issues, String(bad)).toEqual([
        { path: "returnUrl", message: "returnUrl must be a string" },
      ]);
      expect(fetchFn, String(bad)).not.toHaveBeenCalled();
    }
  });
});
...
describe("checkout.retrieve", () => {
  ...
  it.each([other, undefined, null, "", 123])(
    "MR-102: refuses an authenticated checkout destination outside the pin (%s)",
    async (destinationWallet) => {
      const client = new GenesisPay({
        apiKey: TEST_KEY,
        expectedPayTo: receiving,
        fetchFn: jsonFetch({ link: sessionLink({ destinationWallet }) }),
      });

      await expect(client.checkout.retrieve("p1")).rejects.toBeInstanceOf(
        GenesisPayNetworkSafetyError,
      );
    },
  );

  it.each([undefined, receiving])("maps the authenticated checkout with expectedPayTo=%s", async (expectedPayTo) => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
      expectedPayTo,
  ...
  it("raises GenesisPayNotFoundError on 404 — polling an unknown id is not a config error", async () => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
    ...
    expect(error).toBeInstanceOf(GenesisPayNotFoundError);
    expect(error).not.toBeInstanceOf(GenesisPayConfigError);
    expect((error as Error).name).toBe("GenesisPayNotFoundError");
  });

  it("raises GenesisPayConfigError on 401 and 500", async () => {
    for (const status of [401, 500]) {
      const client = new GenesisPay({
        apiKey: TEST_KEY,
        fetchFn: jsonFetch({ error: "boom" }, status) as unknown as typeof fetch,
      });
      await expect(client.checkout.retrieve("p1")).rejects.toBeInstanceOf(GenesisPayConfigError);
    }
  });

  it("raises GenesisPayConfigError when the transport itself fails", async () => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
      fetchFn: (async () => {
  ...
  it("rejects an empty publicId before touching the network", async () => {
    ...
    await expect(client.checkout.retrieve("   ")).rejects.toBeInstanceOf(GenesisPayConfigError);
    expect(fetchFn).not.toHaveBeenCalled();
  });
...
describe("typed HTTP errors", () => {
  ...
  it.each([409, 422])(
    "preserves strict checkout error correlation metadata on HTTP %s",
    async (status) => {
      const client = new GenesisPay({
        apiKey: TEST_KEY,
        fetchFn: strictFetch(validationBody, status) as unknown as typeof fetch,
      });
      const error = await client.checkout
        .create({ title: "x", amountUsdc: "1.00" }, CREATION_OPTIONS)
        .catch((value: unknown) => value);

      expect(error).toBeInstanceOf(
        status === 422 ? GenesisPayValidationError : GenesisPayConfigError,
      );
      expect(error).toMatchObject({
        requestId: STRICT_REQUEST_ID,
        apiVersion: "2026-08-26",
        ...(status === 422 ? { issues: validationBody.issues } : {}),
      });
    },
  );

  it.each(["headers", "body"] as const)(
    "retains checkout error metadata when only the %s survive",
    async (source) => {
      const metadata = { requestId: STRICT_REQUEST_ID, apiVersion: "2026-08-26" };
      const client = new GenesisPay({
        apiKey: TEST_KEY,
        fetchFn: errorFetch(
          { error: "Idempotency conflict", ...(source === "body" ? metadata : {}) },
          409,
          source === "headers" ? {
            "GENESISPAY-Request-Id": STRICT_REQUEST_ID,
            "GENESISPAY-Version": "2026-08-26",
      ...
      await expect(client.checkout.create(
        { title: "x", amountUsdc: "1.00" }, CREATION_OPTIONS,
      )).rejects.toMatchObject({ name: "GenesisPayConfigError", ...metadata });
    },
  );

  it("retains checkout error headers when the response stream fails", async () => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
    ...
    await expect(client.checkout.create(
      ...
      name: "GenesisPayConfigError",
      requestId: STRICT_REQUEST_ID,
      apiVersion: "2026-08-26",
    });
  });

  it("raises GenesisPayValidationError with the server issues on 422 (create)", async () => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
    ...
    expect(error).toBeInstanceOf(GenesisPayValidationError);
    expect(error).not.toBeInstanceOf(GenesisPayConfigError);
    expect((error as GenesisPayValidationError).name).toBe("GenesisPayValidationError");
    expect((error as GenesisPayValidationError).issues).toEqual(validationBody.issues);
    expect((error as Error).message).toContain("Payment link input is invalid.");
  });

  it("raises GenesisPayValidationError on 422 for retrieve as well — one mapping for every method", async () => {
    const client = new GenesisPay({ apiKey: TEST_KEY, fetchFn: errorFetch(validationBody, 422) });

    const error = await client.checkout.retrieve("p1").catch((e: unknown) => e);

    expect(error).toBeInstanceOf(GenesisPayValidationError);
    expect((error as GenesisPayValidationError).issues).toHaveLength(2);
  });
  ...
  it("raises GenesisPayRateLimitError with Retry-After on 429", async () => {
    const client = new GenesisPay({
      apiKey: TEST_KEY,
    ...
    expect(error).toBeInstanceOf(GenesisPayRateLimitError);
    expect((error as GenesisPayRateLimitError).name).toBe("GenesisPayRateLimitError");
    expect((error as GenesisPayRateLimitError).retryAfterSeconds).toBe(17);
  ...
  it("reports retryAfterSeconds = null rather than guessing when the header is missing", async () => {
    ...
    expect(error).toBeInstanceOf(GenesisPayRateLimitError);
    expect((error as GenesisPayRateLimitError).retryAfterSeconds).toBeNull();
  });
  ...
  it("rate-limits the seller lookup too — a 429 is not a broken key", async () => {
    ...
    expect(error).toBeInstanceOf(GenesisPayRateLimitError);
    expect((error as GenesisPayRateLimitError).retryAfterSeconds).toBe(5);
  });

  it("keeps 404 on GenesisPayNotFoundError and everything else on GenesisPayConfigError", async () => {
    const notFound = new GenesisPay({
      apiKey: TEST_KEY,
      fetchFn: errorFetch({ error: "Payment link not found." }, 404),
    });
    await expect(notFound.checkout.retrieve("p1")).rejects.toBeInstanceOf(GenesisPayNotFoundError);
    ...
    for (const status of [400, 401, 403, 409, 500, 503]) {
      ...
      expect(error).not.toBeInstanceOf(GenesisPayValidationError);
      expect(error).not.toBeInstanceOf(GenesisPayRateLimitError);
    }
  });
...
describe("checkout.simulatePayment", () => {
  ...
  it("names the test-key requirement on 403", async () => {
    const client = new GenesisPay({
      ...
      baseUrl: "https://prod.example",
      fetchFn: simulateFetch(
        { error: "simulate-payment requires a test-mode API key (gp_sk_test_…)." },
        403,
      ) as unknown as typeof fetch,
    });
  ...
  it("names both causes of a 404 — missing link or a mainnet deployment", async () => {
    ...
    expect((error as Error).message).not.toContain("gp_sk_test_");
  });
  ...
  it("uses the shared mapping for 422 and 429 like every other method", async () => {
    ...
    const validationError = await invalid.checkout
      ...
      .catch((e: unknown) => e);
    expect(validationError).toBeInstanceOf(GenesisPayValidationError);
    expect((validationError as GenesisPayValidationError).issues).toEqual([
      { path: "payerWallet", message: "payerWallet must be a valid EVM wallet address" },
    ]);

    const limited = new GenesisPay({
      apiKey: TEST_KEY,
    ...
    const rateLimitError = await limited.checkout
      .simulatePayment("p1")
      .catch((e: unknown) => e);
    expect(rateLimitError).toBeInstanceOf(GenesisPayRateLimitError);
    expect((rateLimitError as GenesisPayRateLimitError).retryAfterSeconds).toBe(9);
  });
  ...
  it("throws when the 201 body carries no link or no attempt", async () => {
    ...
    const noAttempt = new GenesisPay({
      apiKey: TEST_KEY,
