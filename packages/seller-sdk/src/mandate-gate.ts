 ...
 * caller presents a standing payment mandate id via the `PEERPAY-MANDATE`
 * header. The gate debits the mandate through PeerPay
...
export const MANDATE_HEADER = "PEERPAY-MANDATE";

export type MandateGateOptions = {
  /** Decimal charge per request, e.g. "0.08". */
  amount: string;
  /** Base URL of the PeerPay app (your PEERPAY_BASE_URL). */
  baseUrl: string;
...
export type MandateGate = {
  ...
  /**
   * Wraps a fetch-style handler: requests carrying a valid `PEERPAY-MANDATE`
...
export function createMandateGate(options: MandateGateOptions): MandateGate {
  ...
  if (!apiKey) {
    throw new Error("createMandateGate requires a PeerPay seller API key (pp_sk_...).");
  }

  const baseUrl = options.baseUrl?.trim().replace(/\/+$/, "");
  if (!baseUrl) {
    throw new Error("createMandateGate requires the PeerPay base URL.");
  }
  ...
  async function charge(
    ...
    try {
      ...
      return {
        ok: false,
        status: 503,
        error: `PeerPay is unreachable: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
      ...
      return {
        ...
        error:
          typeof record.error === "string"
            ? record.error
            : `PeerPay rejected the mandate charge (${response.status}).`,
      };
    }
  ...
  function wrap(handler: (request: Request) => Response | Promise<Response>) {
    return async (request: Request): Promise<Response> => {
        ...
        return jsonResponse(
          {
            ...
            howToPay: `Attach an active PeerPay mandate id via the ${MANDATE_HEADER} header, or pay per call via x402.`,
            amount: options.amount,
          },
          402,
        );
      ...
      headers.set("PEERPAY-MANDATE-CHARGE", outcome.charge.id);
