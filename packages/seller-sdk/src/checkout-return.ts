 ...
 * payer can open that URL without paying. Use the seller-authenticated strict
 * `fulfillment.verify` authority path before fulfilment — never these fields.
 */
 ...
 * `fulfillment.verify({ locator: { linkId: hint.linkId }, expected })`
 * (authenticated with your seller key) before acting on it. A link locator is
 * suitable only for a single-use link; reusable links require an attempt ID.
 */
