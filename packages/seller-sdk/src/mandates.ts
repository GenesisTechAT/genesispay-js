// `peerpay.mandates.*` — payment mandates over `/api/v1/mandates`.
//
...
import { PeerPayConfigError } from "./errors.js";
import {
  asRecord,
...
export interface MandatesResource {
   ...
   * mandate of this account is refused by the server (as a `PeerPayConfigError`
...
export function createMandatesResource(request: PeerPayRequest): MandatesResource {
  return {
    create: async (input) => {
      ...
      if (
        ...
        throw new PeerPayConfigError(
          `PeerPay POST /api/v1/mandates returned a mandate without permitTypedData: ${JSON.stringify(body)}`,
        );
      }
      ...
      if (!spender) {
        ...
        throw new PeerPayConfigError(
          `PeerPay POST /api/v1/mandates returned a mandate without a spender: ${JSON.stringify(body)}`,
        );
      }
    ...
    list: async (opts = {}) => {
      ...
      if (!Array.isArray(mandates)) {
        ...
        throw new PeerPayConfigError(
          `PeerPay GET /api/v1/mandates returned no mandates array: ${JSON.stringify(body)}`,
        );
      }
    ...
    charge: async (id, input) => {
      ...
      if (!raw || typeof raw.id !== "string" || !raw.id) {
        throw new PeerPayConfigError(
          `PeerPay POST /api/v1/mandates/${mandateId}/charge returned no charge: ${JSON.stringify(body)}`,
        );
      }
      return {
        id: raw.id,
...
function notFoundMessage(id: string): string {
  return (
    `No PeerPay mandate with id "${id}". Check the id and that it belongs to ` +
...
function requireMandate(body: unknown, operation: string): Mandate {
  ...
  if (!raw || typeof raw.id !== "string" || !raw.id) {
    throw new PeerPayConfigError(
      `PeerPay ${operation} returned no mandate: ${JSON.stringify(body)}`,
    );
  }
