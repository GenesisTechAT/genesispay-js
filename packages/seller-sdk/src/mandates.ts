...
import { proposeContractUsage, submitContractUsage, getContractUsage, type ContractUsageCharge, type ContractUsageInput } from "./contract-usage.js";
import type { Hex } from "viem";
import { createContractPerUse, createContractSubscription, type ContractPerUseCreateInput, type ContractPerUseCreateResult,
  type ContractSubscriptionCreateInput, type ContractSubscriptionCreateResult } from "./contract-mandates.js";
...
export type MandateChargeStatus = "pending" | "submitted" | "settled" | "failed";

export type Mandate = {
  protocol?: "contract_mandate_v1";
  activationStatus?: "proposed" | "submitted" | "confirmed";
  revocationStatus?: "none" | "requested" | "confirmed";
  executor?: string;
  ...
  createdAt: string;
  /** Set after canonical activation confirmation. HTTP acceptance is not activation. */
...
export type MandateChargeInput = {
  ...
  amount: string;
  /** Stable identity for this logical usage request; reuse it after timeouts. */
...
function requireMandateChargeIdempotencyKey(value: unknown): string {
  if (
    typeof value !== "string" ||
    ...
    throw new GenesisPayValidationError(
      "mandates.charge requires an idempotencyKey with 8–200 safe characters.",
...
export interface MandatesResource {
  ...
  /** Explicit two-signature contract protocol. New capabilities remain gated by the deployment. */
  createContractSubscription(input: ContractSubscriptionCreateInput): Promise<ContractSubscriptionCreateResult>;
  /** Bounded per-use consent; every later charge needs a separate payer signature. */
  createContractPerUse(input: ContractPerUseCreateInput): Promise<ContractPerUseCreateResult>;
  proposeContractUsage(input: ContractUsageInput): Promise<ContractUsageCharge>;
  submitContractUsage(proposal: ContractUsageCharge, signature: Hex): Promise<ContractUsageCharge>;
  getContractUsage(proposal: ContractUsageCharge): Promise<ContractUsageCharge>;
  /**
  ...
  /** Requests cancellation. Contract mandates require confirmed payer revocation before authority is extinguished. */
  revoke(id: string): Promise<Mandate>;
}

export function createMandatesResource(request: GenesisPayRequest): MandatesResource {
  return {
    createContractSubscription: (input) => createContractSubscription(request, input),
    createContractPerUse: (input) => createContractPerUse(request, input),
    proposeContractUsage: (input) => proposeContractUsage(request, input),
    submitContractUsage: (proposal, signature) => submitContractUsage(request, proposal, signature),
    getContractUsage: (proposal) => getContractUsage(request, proposal),
    create: async (input) => {
    ...
    charge: async (id, input) => {
      ...
      const idempotencyKey = requireMandateChargeIdempotencyKey(
      ...
      return {
        ...
        status: oneOf(
          raw.status,
          ["pending", "submitted", "settled", "failed"] as const,
          "pending",
        ),
...
export function toMandate(raw: Record<string, unknown>): Mandate {
  return {
    ...(raw.protocol === "contract_mandate_v1" ? { protocol: "contract_mandate_v1" as const,
      activationStatus: oneOf(raw.activationStatus, ["proposed", "submitted", "confirmed"] as const, "proposed"),
      revocationStatus: oneOf(raw.revocationStatus, ["none", "requested", "confirmed"] as const, "none"),
      executor: requiredString(raw.executor), contractVersion: requiredString(raw.contractVersion) } : {}),
    ...
    status: oneOf(
      raw.status,
      ["pending_permit", "active", "past_due", "revoked", "expired", "cancelled"] as const,
      "pending_permit",
    ),
