...
describe("mandates facade", () => {
  it("offers no activate — that step carries the payer's signature, not this key", () => {
    ...
    expect(Object.keys(client.mandates).sort()).toEqual([
      "charge",
      "create",
      "createContractPerUse",
      "createContractSubscription",
      "getContractUsage",
      "list",
      "proposeContractUsage",
      "retrieve",
      "revoke",
      "submitContractUsage",
    ]);
...
describe("mandates.create", () => {
  ...
  it("raises GenesisPayValidationError with the server issues on 422", async () => {
  ...
  it("surfaces a 503 (facilitator not configured) as a config error with the server text", async () => {
    const client = clientFor(
      jsonFetch(
        { error: "The GenesisPay facilitator is not configured, so mandates cannot be created." },
        503,
      ),
    );
...
describe("mandates.list", () => {
  ...
  it("preserves terminal cancelled authority as a known mandate status", async () => {
    const client = clientFor(
      jsonFetch({ mandates: [mandateView({ status: "cancelled" })], hasMore: false }),
    );

    const page = await client.mandates.list();

    expect(page.mandates[0]?.status).toBe("cancelled");
  });
  ...
  it("raises GenesisPayValidationError with the issues on a 400 query rejection", async () => {
    const client = clientFor(
      jsonFetch(
        {
  ...
  it("keeps a 400 without field issues on GenesisPayConfigError", async () => {
    const client = clientFor(
  ...
  it("raises GenesisPayRateLimitError with Retry-After on 429", async () => {
    ...
    expect((error as GenesisPayRateLimitError).retryAfterSeconds).toBe(7);
  });
});
...
describe("mandates.retrieve", () => {
  ...
  it("URL-encodes the id and raises GenesisPayNotFoundError on 404", async () => {
  ...
  it("rejects an empty id before touching the network", async () => {
    ...
    await expect(client.mandates.retrieve("  ")).rejects.toBeInstanceOf(GenesisPayConfigError);
    expect(fetchFn).not.toHaveBeenCalled();
  });
  ...
  it("throws when the 200 body carries no mandate", async () => {
    ...
    await expect(client.mandates.retrieve("mnd_1")).rejects.toBeInstanceOf(GenesisPayConfigError);
  });
});
...
describe("mandates.charge", () => {
  it("POSTs the amount and maps the settled charge", async () => {
    ...
    const charge = await client.mandates.charge("mnd_1", {
      amount: "0.08",
      idempotencyKey: "usage-request-123",
    });
  ...
  it("maps a charge that has no txHash yet to null", async () => {
    ...
    await expect(client.mandates.charge("mnd_1", {
      amount: "0.08",
      idempotencyKey: "usage-request-123",
  ...
  it("MR-202/MR-406/MR-1106: preserves the typed 503 charge locator", async () => {
    ...
    const error = await client.mandates.charge("mnd_1", {
      amount: "0.08",
      idempotencyKey: "usage-request-123",
    }).catch((caught: unknown) => caught);

    expect(error).toBeInstanceOf(GenesisPayMandateChargePendingError);
  ...
  it("MR-404/MR-406: rejects a missing runtime key before it becomes the literal undefined header", async () => {
    const fetchFn = jsonFetch({
      charge: { id: "chg_1", status: "settled", amountMinor: "80000", txHash: "0xabc" },
    });
    const client = clientFor(fetchFn);

    const error = await client.mandates.charge(
      "mnd_1",
      { amount: "0.08" } as Parameters<typeof client.mandates.charge>[1],
    ).catch((caught: unknown) => caught);

    expect(error).toBeInstanceOf(GenesisPayValidationError);
    expect(error).toMatchObject({
      issues: [{ path: "idempotencyKey", message: "invalid idempotency key" }],
    });
    expect(fetchFn).not.toHaveBeenCalled();
  });

  it("raises GenesisPayNotFoundError for an unknown mandate", async () => {
    const client = clientFor(
      jsonFetch({ error: "Mandate not found.", code: "mandate_not_found" }, 404),
    );

    const error = await client.mandates
      .charge("mnd_missing", { amount: "0.08", idempotencyKey: "usage-request-123" })
      .catch((e: unknown) => e);
  ...
  it("surfaces a refused charge (cap or allowance) as a config error with the server text", async () => {
    ...
    const error = await client.mandates
      .charge("mnd_1", { amount: "5", idempotencyKey: "usage-request-123" })
      .catch((e: unknown) => e);
  ...
  it("raises GenesisPayRateLimitError with Retry-After on 429", async () => {
    ...
    const error = await client.mandates
      .charge("mnd_1", { amount: "0.08", idempotencyKey: "usage-request-123" })
      .catch((e: unknown) => e);

    expect(error).toBeInstanceOf(GenesisPayRateLimitError);
    expect((error as GenesisPayRateLimitError).retryAfterSeconds).toBe(3);
  });
  ...
  it("rejects an empty id before touching the network", async () => {
    ...
    await expect(client.mandates.charge("", { amount: "1", idempotencyKey: "usage-request-123" })).rejects.toBeInstanceOf(
      GenesisPayConfigError,
    );
    expect(fetchFn).not.toHaveBeenCalled();
  });
  ...
  it("throws when the 200 body carries no charge", async () => {
    ...
    await expect(
      client.mandates.charge("mnd_1", { amount: "0.08", idempotencyKey: "usage-request-123" }),
    ).rejects.toBeInstanceOf(GenesisPayConfigError);
  });
});
...
describe("mandates.revoke", () => {
  ...
  it("raises GenesisPayNotFoundError on 404", async () => {
  ...
  it("rejects an empty id before touching the network", async () => {
    ...
    await expect(client.mandates.revoke("   ")).rejects.toBeInstanceOf(GenesisPayConfigError);
    expect(fetchFn).not.toHaveBeenCalled();
  });
});
