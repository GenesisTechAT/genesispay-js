...
import { createCheckoutDocumentsResource } from "./checkout-documents.js";
import { parseItemTax, type ItemTax, type ItemTaxUpdate } from "./item-tax.js";
import type { CheckoutDocumentsResource } from "./checkout-documents.js";
...
/**
 * The `GenesisPay` client — a Stripe-like entry point that owns the seller key and
...
export type GenesisPayClientOptions = {
  ...
  configTtlMs?: number;
  /**
   * Optional local payout pin, including checkout retrieval and strict fulfillment
   * verification. Historical reconciliation must use the original destination's pin.
   */
...
export type CheckoutCreateInput = CheckoutAmountInput & {
  ...
  title: string;
  description?: string;
  /** Payment requests are single-use. Use products for repeat purchases. */
  linkType?: "single";
...
export type IdempotentCreationOptions = {
  /** Seller-account-scoped retry identity; 1–255 characters without surrounding whitespace. */
...
export type CheckoutAttempt = {
   ...
   *
   * Defaults to `false` only on this legacy display/polling view when an older
   * backend omits it. This value is never fulfillment evidence; use
   * `fulfillment.verify()` for versioned real-payment authority.
   */
  simulated: boolean;
};
...
function requireIdempotencyKey(
  options: IdempotentCreationOptions | undefined,
  operation: string,
): string {
  ...
  if (
    ...
    throw new GenesisPayValidationError(
      `${operation} requires an idempotencyKey with 1–255 characters and no surrounding whitespace.`,
...
export class GenesisPay {
  ...
  readonly checkout: {
    create(input: CheckoutCreateInput, options: IdempotentCreationOptions): Promise<CheckoutLink>;
    updateTax(publicId: string, input: ItemTaxUpdate): Promise<CheckoutLink>;
  ...
  /** Immutable documents generated after hosted human checkout completes. */
  readonly checkoutDocuments: CheckoutDocumentsResource;
  /** Strict seller-scoped payment authority. Tolerant checkout views are display-only. */
  readonly fulfillment: FulfillmentResource;
  /**
  ...
  constructor(options: GenesisPayClientOptions) {
    ...
    this.checkout = {
      create: (input, creationOptions) => this.createCheckout(input, creationOptions),
      updateTax: async (publicId, input) => {
        if (!publicId.trim()) throw new GenesisPayConfigError("A payment link publicId is required.");
        const body = await this.request({ method: "PATCH", path: `/api/v1/links/${encodeURIComponent(publicId)}`, operation: "PATCH /api/v1/links/:publicId", action: "update item tax", body: { taxConfig: parseItemTax(input.taxConfig), expectedTaxConfig: parseItemTax(input.expectedTaxConfig) } });
        const link = asRecord(asRecord(body)?.link);
        if (!link) throw new GenesisPayConfigError("Item tax update returned no payment link.");
        this.assertPinnedDestination(link.destinationWallet, "checkout link");
    ...
    this.fulfillment = createFulfillmentResource({
      baseUrl: this.baseUrl,
      apiKey: this.apiKey,
      fetchFn: this.fetchFn,
      assertPinnedDestination: (wallet, what) => this.assertPinnedDestination(wallet, what),
    });
    ...
    this.products = createProductsResource(
      ...
      this.baseUrl,
      (input) => this.fulfillment.verify(input),
      (publicId, body) => this.performProductGateRequest(publicId, body),
      (input) => this.performStrictSellerRequest(input),
    );
    ...
    this.checkoutDocuments = createCheckoutDocumentsResource(this.request);
  ...
  private async performStrictSellerRequest(input: {
    method?: "GET" | "POST";
    path: string;
    body?: unknown;
    ...
    try {
      return await this.fetchFn(`${this.baseUrl}${input.path}`, {
      ...
      throw new GenesisPayConfigError(
        `Failed to reach GenesisPay for a strict seller request: ${describeError(error)}`,
      );
    }
  }
  ...
  /**
   * Applies the `expectedPayTo` pin to a frozen or explicitly expected destination
   * on creation and authenticated retrieval alike (MR-102/MR-202).
   *
  ...
  private assertPinnedDestination(wallet: unknown, what: string): void {
    ...
    if (typeof wallet !== "string" || !wallet) {
      ...
      throw new GenesisPayNetworkSafetyError(
        `The ${what} carried no destination to check against expectedPayTo.`,
      );
    }
    if (wallet.toLowerCase() !== this.expectedPayTo) {
      throw new GenesisPayNetworkSafetyError(
        `The ${what} destination does not match expectedPayTo.`,
      );
    }
  }
  ...
  /**
   * Creates a payable checkout link — and refuses to hand one back unless the
   * backend that created it answered in this SDK's API version.
   *
   * The version header alone is a request, not a negotiation: a backend that
   * predates the strict surface (or one that was rolled back) ignores it, mints
   * a legacy link, and returns a perfectly ordinary 201. That link is payable
   * and its attempt carries no authority provenance, so strict fulfillment can
   * never verify it — the payer's money would be stranded in an unfulfillable
   * purchase. So the raw response is read here rather than through the tolerant
   * shared seam, and the version + request ID are checked before the link
   * becomes a payUrl anyone can hand to a payer.
   */
  private async createCheckout(
    input: CheckoutCreateInput,
    options: IdempotentCreationOptions,
  ): Promise<CheckoutLink> {
    const idempotencyKey = requireIdempotencyKey(options, "checkout.create");
    // Runtime validation also protects JavaScript callers and older integrations.
    if (input.linkType !== undefined && input.linkType !== "single") {
      throw new GenesisPayValidationError(
        "Payment requests are single-use. Create a product for repeat purchases.",
      );
    }
    const amount = resolveCheckoutAmount(input);
    const { returnUrl, cancelUrl } = validateRedirectFields(input);

    const response = await this.performStrictSellerRequest({
      method: "POST",
      path: "/api/v1/links",
    ...
    if (!response.ok) {
      // Error bodies are not authority; keep best-effort header/body metadata
      // even when a proxy replaces or interrupts the response body. The shared
      // mapper still owns status and issue classification and consumes the original.
      const errorBody = await response.clone().json().catch(() => null) as unknown;
      await throwForErrorResponse(response, {
        operation: "POST /api/v1/links",
        metadata: strictResponseContext(response, errorBody),
      });
    }
    const body = (await response.json().catch(() => null)) as unknown;
    requireStrictResponseMetadata(response, body, "checkout link");
  ...
  private async retrieveCheckout(publicId: string): Promise<CheckoutSession> {
    ...
    // Check the raw field before the tolerant display mapper can default it.
    this.assertPinnedDestination(link.destinationWallet, "checkout link");
