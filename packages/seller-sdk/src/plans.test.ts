...
const clientFor = (fetchFn: ReturnType<typeof jsonFetch>, baseUrl = "http://localhost:3000") =>
  new GenesisPay({ apiKey: TEST_KEY, baseUrl, fetchFn: fetchFn as unknown as typeof fetch });
...
describe("plans.create", () => {
  ...
  it("fails closed when the pin is set and the response omits the wallet entirely", async () => {
    // The bypass-by-omission hole: an intermediary that STRIPS destinationWallet
    // must not read as "matches". Absent is exactly what the pin cannot vouch for.
    const client = new GenesisPay({
      apiKey: TEST_KEY,
      baseUrl: "http://localhost:3000",
      expectedPayTo: receiving,
      fetchFn: jsonFetch(
        { plan: planView({ destinationWallet: undefined }) },
        201,
      ) as unknown as typeof fetch,
    });
  ...
  it("raises GenesisPayRateLimitError with Retry-After on 429", async () => {
    ...
    expect((error as GenesisPayRateLimitError).retryAfterSeconds).toBe(12);
  });
...
describe("plans.list", () => {
  ...
  it("throws when the body has no plans array — that is a broken contract, not 'no plans'", async () => {
    ...
    await expect(client.plans.list()).rejects.toBeInstanceOf(GenesisPayConfigError);
  });
});
...
describe("plans.retrieve", () => {
  ...
  it("raises GenesisPayNotFoundError on 404, naming the id", async () => {
  ...
  it("rejects an empty publicId before touching the network", async () => {
    ...
    await expect(client.plans.retrieve("   ")).rejects.toBeInstanceOf(GenesisPayConfigError);
  ...
  it("throws when the 200 body carries no plan", async () => {
    ...
    await expect(client.plans.retrieve("plan_1")).rejects.toBeInstanceOf(GenesisPayConfigError);
  });
});
...
describe("plans.archive", () => {
  ...
  it("rejects an empty publicId before touching the network", async () => {
    ...
    await expect(client.plans.archive("")).rejects.toBeInstanceOf(GenesisPayConfigError);
    expect(fetchFn).not.toHaveBeenCalled();
  });
});
