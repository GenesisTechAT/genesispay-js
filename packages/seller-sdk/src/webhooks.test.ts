...
async function expectRejection(
  ...
  messageMatch: RegExp,
): Promise<PeerPaySignatureVerificationError> {
  let caught: unknown;
  try {
    await promise;
  } catch (error) {
  ...
  expect(caught).toBeInstanceOf(PeerPaySignatureVerificationError);
  const typed = caught as PeerPaySignatureVerificationError;
  expect(typed.name).toBe("PeerPaySignatureVerificationError");
...
describe("constructEvent", () => {
  ...
  it("rejects a missing or empty header", async () => {
    await expectRejection(
      ...
      /missing peerpay-signature header/i,
    );
    await expectRejection(
      constructEvent(RAW_BODY, "   ", SECRET, { nowMs: NOW_MS }),
      /missing peerpay-signature header/i,
...
describe("typed event union", () => {
  ...
  it("narrows data on `event.type === \"payment.confirmed\"` without a cast", async () => {
    ...
    // `data` is narrowed to PeerPayPaymentEventData. `tsc -p packages/seller-sdk`
  ...
  it("tells a mandate.active handler which plan was subscribed to", async () => {
    ...
    // Compiles only because `data` narrows to PeerPayMandateEventData and the
  ...
  it("accepts an unknown event type instead of throwing — a new server event must never 400 a customer's route", async () => {
    ...
    const anyEvent: PeerPayAnyEvent = event;
    expect(anyEvent.type).toBe("invoice.finalized.v7");
    expect(anyEvent.data).toEqual({
      invoice: { id: "inv_1" },
      occurredAt: "2026-07-21T10:00:00.000Z",
    });
    expect(isKnownPeerPayEventType(anyEvent.type)).toBe(false);
  });
  ...
  it("reports which types this SDK version knows", () => {
    expect(PEERPAY_EVENT_TYPES).toContain("payment.confirmed");
    expect(PEERPAY_EVENT_TYPES).toContain("subscription.past_due");
    expect(new Set(PEERPAY_EVENT_TYPES).size).toBe(PEERPAY_EVENT_TYPES.length);
    expect(isKnownPeerPayEventType("link.paid")).toBe(true);
    expect(isKnownPeerPayEventType("link.paid.v2")).toBe(false);
  });

  it("keeps PeerPayEventType and the runtime list in lockstep", () => {
    // Compile-time half: every literal in the runtime list is a PeerPayEventType
    // and every PeerPayEventType is coverable by the list.
    const fromList: PeerPayEventType = PEERPAY_EVENT_TYPES[0];
    const coverage: Record<PeerPayEventType, true> = {
    ...
    expect(Object.keys(coverage).sort()).toEqual([...PEERPAY_EVENT_TYPES].sort());
  });

  it("lets a handler switch exhaustively and still see unhandled types", () => {
    const handled: string[] = [];
    const handle = (event: PeerPayAnyEvent): string => {
      if (!isKnownPeerPayEventType(event.type)) {
        handled.push(`unknown:${event.type}`);
        return "ignored";
      }
      const known = event as PeerPayEvent;
      switch (known.type) {
