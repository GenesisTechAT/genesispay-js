...
describe("peerPaySettlement", () => {
  it("rejects missing API keys and invalid base URLs at creation time", () => {
    expect(() =>
      peerPaySettlement({ facilitatorBaseUrl: "https://peerpay.example", apiKey: " " }),
    ).toThrow("seller API key");
    expect(() =>
      peerPaySettlement({ facilitatorBaseUrl: "not a url", apiKey: "pp_sk_x" }),
  ...
  it("defaults to the development facilitator when facilitatorBaseUrl is omitted", async () => {
    ...
    const verify = peerPaySettlement({
      apiKey: "pp_sk_test",
  ...
  it("POSTs the settle contract and returns the parsed settlement on success", async () => {
    const fetchFn = vi.fn(async () => Response.json(successSettlement));
    const verify = peerPaySettlement({
      facilitatorBaseUrl: "https://peerpay.example/",
      apiKey: "pp_sk_test",
      fetchFn: fetchFn as unknown as typeof fetch,
    });
    ...
    expect(url).toBe("https://peerpay.example/api/v1/facilitator/settle");
  ...
  it("handles unreachable facilitators and non-JSON responses", async () => {
    const unreachable = peerPaySettlement({
      facilitatorBaseUrl: "https://peerpay.example",
      apiKey: "pp_sk_test",
      fetchFn: (async () => {
    ...
    const nonJson = peerPaySettlement({
      facilitatorBaseUrl: "https://peerpay.example",
      apiKey: "pp_sk_test",
      fetchFn: (async () =>
